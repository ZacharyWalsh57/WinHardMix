...
//Include SW Serial setup for communication of the ESP And ATMEGA.  Hardwire is proving to just be a pain in the ass to deal with.
...
//SWSerial setup for the ESP Module.
#include <SoftwareSerial.h>
#define ATMEGA_OUT_TX 2
#define ATMEGA_OUT_RX 3
SoftwareSerial ESPSERIAL(ATMEGA_OUT_RX, ATMEGA_OUT_TX);
...
void setup() {
    ...
    //START serial to send data to the ESP and normal serial output.
    Serial.begin(115200);
    ESPSERIAL.begin(115200);
    delay(5000);
    ESPSERIAL.println("TESTING ESP COMMS");

    //Clear the display and setup the monitor template.
    LCD_DISPLAY.clear(); 
...
void loop() {
    ...
    for (SELECTED = 0; SELECTED < 4; SELECTED = SELECTED + 1) {
        ...
        Serial.print(" ");
        ESPSERIAL.print("<" + VOLUMES[SELECTED] + ">");
        
    }
    Serial.println("");
    ESPSERIAL.println("");
    //This loop shows "CH1 CH2 CH3 CH4" to the serial console on FTDI.  When sent to the ESP, I want to
    //use some sort of faster method to store incoming data
